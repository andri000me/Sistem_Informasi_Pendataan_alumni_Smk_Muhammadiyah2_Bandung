...
*/
	...
	*/
	elseif ($aksi == "tambah_anggota") {
		...
		// Data Terkini
		$posisi         = $_POST["posisi"];
		$alamat_prsh    = $_POST["alamat_perusahaan"];
		$nama_ptn       = $_POST["nama_ptn"];
		$program_std    = $_POST["program_std"];
		$jurusan    	= $_POST["jurusan"];
		$nama_wrs       = $_POST["nama_wrs"];
		...
		if ($sukses_upload) {
			...
			if ($id_anggota!="") {
				...
				// Simpan akademik
				$query = "INSERT INTO aluni_anggota_akademik VALUES ('$id_anggota', '$nis', '$angkatan', '$tahun_masuk', '$tahun_keluar', '$kelas_terakhir', '$pk', '$catatan', '$_SESSION[a2_username]', NOW(), '$_SESSION[a2_username]', NOW(), '0')";
				$proses     = $db->query($query);

				// Simpan terkini
				$query = "INSERT INTO aluni_anggota_terkini VALUES ('$id_anggota','$posisi' ,'$alamat_prsh', '$nama_ptn', '$program_std', '$jurusan', '$nama_wrs', '$jenis_usaha', '$alamat_usaha')";
				$proses     = $db->query($query);
			}
	...
	*/
	elseif ($aksi == "ubah_anggota") {
		...
		if ($sukses_upload) {
			if ($id_anggota!="") {
				...
				// Simpan akademik
				$query = "UPDATE aluni_anggota_akademik SET nis = '$nis', angkatan = '$angkatan', tahun_masuk = '$tahun_masuk', tahun_keluar = '$tahun_keluar', kelas_terakhir = '$kelas_terakhir', catatan = '$catatan', kode_pk = '$kode_pk', updated_by = '$_SESSION[a2_username]', updated_date = NOW(), revisi = revisi+1 WHERE id_anggota = '$id_anggota'";
				$proses = $db->query($query);

				// Simpan Pengguna
				$query  = "UPDATE aluni_anggota_terkini SET posisi = '$posisi', alamat_perusahaan='$alamat_usaha', nama_ptn='$nama_ptn', program_std='$program_std', jurusan='$jurusan', nama_usaha='$nama_wrs', jenis_usaha='$jenis_usaha', alamat_usaha='$alamat_usaha'  WHERE id_anggota = '$id_anggota'";
				$proses = $db->query($query);
	...
	elseif ($aksi == "tambah_pk") {
		...
		// Set Query
		if ($pk!="") {
			$query  = "INSERT INTO aluni_program_keahlian VALUES ('', '$pk','$des')";
			$proses = $db->query($query);

			if ($proses) {
				$_SESSION["informasi"] = "Program Keahlian telah berhasil disimpan.";
			}
			else {
				$_SESSION["informasi"] = "Program Keahlian gagal disimpan!";
			}
		}

		// Kembalikan
		header("Location: ./program_keahlian.php");
		die();
	}
	...
	elseif ($aksi == "ubah_pk") {
		...
		// Set Query
		if ($pk!="") {
			$query  = "UPDATE aluni_program_keahlian SET pk = '$pk', deskripsi = '$des' WHERE kode_pk = '$kode_pk'";
			$proses = $db->query($query);

			if ($proses) {
				$_SESSION["informasi"] = "Program Keahlian telah berhasil dirubah.";
			}
			else {
				$_SESSION["informasi"] = "Program Keahlian gagal dirubah!";
			}
		}
	...
	*/
	...
	elseif ($aksi == "hapus_anggota") {
		$id_anggota = $_POST["id_anggota"];
		$query  = "SELECT username FROM aluni_pengguna WHERE id_anggota = '$id_anggota'";
			$datas  = $db->query($query);
		...
		$proses   = $db->query($query);
		// Hapus pengguna status password
		$query    = "DELETE FROM aluni_pengguna_status_password WHERE username = '$username'";
		$proses   = $db->query($query);
		// Hapus Anggota
		$query    = "DELETE FROM aluni_anggota_dasar WHERE id_anggota = '$id_anggota'";
		$proses   = $db->query($query);
		$query    = "DELETE FROM aluni_anggota_akademik WHERE id_anggota = '$id_anggota'";
		$proses   = $db->query($query);
		$query    = "DELETE FROM aluni_anggota_keluarga WHERE id_anggota = '$id_anggota'";
		$proses   = $db->query($query);
		$query    = "DELETE FROM aluni_anggota_kontak WHERE id_anggota = '$id_anggota'";
		$proses   = $db->query($query);
		$query    = "DELETE FROM aluni_anggota_orang_tua WHERE id_anggota = '$id_anggota'";
		$proses   = $db->query($query);
		$query    = "DELETE FROM aluni_anggota_terkini WHERE id_anggota = '$id_anggota'";
		$proses   = $db->query($query);
